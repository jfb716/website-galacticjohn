

$(document).ready(function(){


/* ------ Hamburger ------*/

if (window.matchMedia("(max-width: 500px)").matches) {
$( ".cross" ).hide();
$( ".navBar" ).hide();
$( ".hamburger" ).click(function() {
$( ".navBar" ).slideToggle( "slow", function() {
$( ".hamburger" ).hide();
$( ".cross" ).show();
});
});

$( ".cross" ).click(function() {
$( ".navBar" ).slideToggle( "slow", function() {
$( ".cross" ).hide();
$( ".hamburger" ).show();
});
});
    console.log("Mobile");
} else {
console.log("Desktop");
}



/* ------ Buttons ------- */
$("button").button();




/* ------ Date ------ */
var d = new Date();
document.getElementById("date").innerHTML = d.toDateString();



/* ------- Weather Temp API ------- */

$.ajax({
  url : "http://api.wunderground.com/api/ecac81007093c564/conditions/q/NY/New_York.json",
  dataType : "jsonp",
  success : function(parsed_json) {
  var temp_f = parsed_json['current_observation']['temp_f'];
$("#temp").html(temp_f);
  }
  });


$.ajax({
  url : "https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20yahoo.finance.quotes%20where%20symbol%20in%20(%22FB%22)%0A%09%09&format=json&env=http%3A%2F%2Fdatatables.org%2Falltables.env&callback=",
  dataType : "json",
  success : function(parsed_json) {
  var price = parsed_json['query']['results']['quote']['Ask'];
$("#stock_price").html(price);
  }
  });


/* ------- Dark Neon Styles off Local Storage ------- */

if (localStorage.getItem("space") === "black") {
  $("body, .navBar, .section_name, .content_box, .ad_box, a").css({
    "background-color": "black",
    "color": "#83F52C",
    "border-color": "#FF6600"
  });

  $("img").css({
    "background-color": "#83F52C"
  });

  $("#sbar").css({
    "background-color": "#FF6600"
  });

  $("#time-field").css({
    "color": "#FF6600"
  });
}


});
